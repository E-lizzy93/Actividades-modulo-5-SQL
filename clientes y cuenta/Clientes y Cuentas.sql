-- Ejercicio 5 — Clientes y Cuentas
...
-- DML solicitado
-- UPDATE: aumentar saldo de la cuenta 402 en 500.00
UPDATE Cuentas
SET saldo = saldo + 500.00
WHERE id_cuenta = 402;

-- DELETE: borrar la cuenta 503
DELETE FROM Cuentas
WHERE id_cuenta = 503;

-- 3) Saldo de cada cuenta del cliente con más años de edad
SELECT c.nombre,
       cu.id_cuenta,
       cu.saldo
FROM Clientes c
...
SELECT AVG(DISTINCT c.edad) AS promedio_edad_clientes_con_saldo_negativo
FROM Clientes c
JOIN Cuentas cu
  ON c.id_cliente = cu.id_cliente
WHERE cu.saldo < 0;
...
-- 7) Todos los clientes y su saldo combinado,
--    considerando solo aquellos con al menos una cuenta negativa
SELECT c.nombre,
       SUM(cu.saldo) AS saldo_combinado
FROM Clientes c
JOIN Cuentas cu
